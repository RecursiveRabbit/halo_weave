        ...
        for idx, filepath in enumerate(token_files):
            ...
            for pos_idx, attn_value in enumerate(aggregated):
                if pos_idx < prompt_length:
                    ...
                    if (current_position - actual_position) < self.parameters['min_distance']:
                        continue
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 voting_strategy.py <capture_dir> [min_distance]")
        sys.exit(1)
    ...
    # Run strategy
    strategy = VotingStrategy(capture_dir, min_distance=min_distance)
